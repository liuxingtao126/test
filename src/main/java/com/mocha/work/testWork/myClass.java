...
public class myClass {
	...
	public static void main(final String[] args) {
		...
		final myClass tree = new myClass();
		final String[] datas = new String[]{"MORSE","T","E","M","N","A","I","O","G","K","D","W","R","U","S"};
		final List<Node> nodelist = new LinkedList<>();
		tree.creatBinaryTree(datas, nodelist);
		final Node root = nodelist.get(0);
		
		System.out.println("获取左节点");
		System.out.println(root.getRight().getRight().var);
		...
		final List<List<String>> rst = new ArrayList<>();
	 ...
	 */
	private void creatBinaryTree(final String[] datas,final List<Node> nodelist){
		//将数组变成node节点
		for(int nodeindex=0;nodeindex<datas.length;nodeindex++){
			final Node node = new Node(datas[nodeindex]);
			nodelist.add(node);
		}
		//给所有父节点设定子节点
		...
		final int index = nodelist.size()/2-1;
	 ...
	 * @param node
	 */
	public void checkCurrentNode(final Node node){
	 ...
	 * @param root 二叉树根节点
	 */
	public void preOrderTraversal(final Node node){
	 ...
	 * @param root 根节点
	 */
	public void postOrderTraversal(final Node node){
		if (node == null)  //很重要，必须加上
            return;  
	 ...
	 * @param node
	 */
	public void preOrderTraversalbyLoop(final Node node){
	 ...
	 * @param node
	 */
	public void inOrderTraversalbyLoop(final Node node){
		final Stack<Node> stack = new Stack();
		Node p = node;
		while(p!=null || !stack.isEmpty()){
			while(p!=null){
	 ...
	 * @param node
	 */
	public void postOrderTraversalbyLoop(final Node node){
		final Stack<Node> stack = new Stack<>();
		...
		while(p!=null || !stack.isEmpty()){
			...
			if(!stack.isEmpty()){
				final Node temp = stack.peek().getRight();
	 ...
	 * @param list
	 */
	public void dfs(final Node node,final List<List<String>> rst,final List<String> list){
		if(node == null) return;
